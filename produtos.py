...
def UpdateProdutos(cdProduto: int, info: Produtos):
    ...
    cursor.execute("""
                   ...
                   WHERE CdProduto = ?
                   """, (int(info.cdProduto)), (int(info.cdStatus)), (str(info.nomeProduto)), (str(info.descricaoProduto)), (float(info.valorProduto)))
...
def DeleteProdutos(cdProduto: int, info: Produtos):
    conn = sqlite3.connect('produtos.db')
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM Produto WHERE CdProduto = ?", str(cdProduto))
        cursor.execute("DELETE * FROM Produto WHERE CdProduto = {0}".format(cdProduto))
    except:
